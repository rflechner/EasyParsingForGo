...
func Optional[T any](parser Parser[T]) Parser[helpers.Option[T]] {
	return func(context ParsingContext) (ParseResult[helpers.Option[T]], error) {
		result, err := parser(context)
		if err != nil {
			return ParseResult[helpers.Option[T]]{
				Result:  helpers.None[T](),
				Context: context,
			}, nil
		}
		return ParseResult[helpers.Option[T]]{
			Result:  helpers.Some(result.Result),
			Context: result.Context,
		}, nil
	}
}
...
func OrElse[T any](parsers ...Parser[T]) Parser[T] {
	return func(context ParsingContext) (ParseResult[T], error) {
		...
		return ParseResult[T]{
			Context: context,
		}, errors.Join(helpers.ListToSlice[error](errorList)...)
	}
}
...
func UntilText[T any](parser Parser[T], delimiter string, includeDelimiter bool) Parser[T] {
	return func(context ParsingContext) (ParseResult[T], error) {
		index := helpers.IndexOf(context.Remaining, delimiter)
		if index < 0 {
			return ParseResult[T]{
				Context: context,
...
func SeparatedBy[A any, B any](parser Parser[A], separator Parser[B], matchTailingSeparator bool) Parser[[]A] {
	return func(context ParsingContext) (ParseResult[[]A], error) {
		...
		if !matchTailingSeparator && hasTailingSeparator {
			return ParseResult[[]A]{
				Context: previous,
				Result:  helpers.ListToSlice[A](l),
			}, nil
		}
		return ParseResult[[]A]{
			Context: next,
...
func Integer() Parser[int] {
	return func(context ParsingContext) (ParseResult[int], error) {
		digitsRes, err := Many(Digit())(context)
		if err != nil {
			return ParseResult[int]{}, err
		}
		if len(digitsRes.Result) == 0 {
			return ParseResult[int]{}, fmt.Errorf("expected at least one digit")
		}
		val, _ := strconv.Atoi(string(digitsRes.Result))
		return ParseResult[int]{
			Result:  val,
			Context: digitsRes.Context,
		}, nil
	}
}
